...
- 1순위 (Cloud): r2_manager.py를 호출하여 R2 내 User_Assets/characters/{id}/ 경로의 metadata.json 및 .md 파싱.
- 2순위 (Local): R2 연결 실패 시 {ROOT_LOCAL_PATH}/characters/{character_id}/의 로컬 데이터 참조.
- 이미지 참조: 나노바나나 전달 시 R2의 Public Development URL을 최우선으로 매핑.
**Camera 호출:** 사용자가 앵글(low_angle) 선택 시
- 데이터 로드: {ROOT_CORE_PATH}/camera_rigs/ (GitHub Sync)에서 표준 리깅 데이터 로드.
- 커스텀 앵글: R2 내 Core_Presets/camera_rigs/rigs_custom/에 별도 설정이 있을 경우 R2 데이터를 우선 적용.
...
6. **R2 모듈화 원칙** — R2 스토리지 연결, 마크다운 파싱, Anthropic API 연동 로직은 반드시 독립된 `r2_manager.py` 파일로 작성한다. 메인 프로그램과는 JSON 인터페이스를 통해 통신한다.
...
- ✅ 파싱 가능한 캐릭터, 카메라 앵글, rig 자료, md 클라우드 업데이트
- 🔄 R2 API 연동으로 이미지, md, 메타데이터 접근 기능
- 🔄 프롬프트 품질 고도화
- 🔄 txt 데이터 채우기 (홀리가 매일 Hex Palette 10개)

### [NEW] R2 에셋 파이프라인 (2026-03-29 업데이트)
- **에셋 로드 방식:** 8GB 소스 전체 스캔 대신 `metadata.json` 인덱싱 기반의 '핀포인트 로드' 방식 채택.
- **이미지 참조:** 나노바나나(Imagen 4) 생성 시 R2의 `Public Development URL`을 Image Reference로 직접 주입.
- **프롬프트 고도화:** R2에서 추출된 HEX 값, 필름 룩 명세를 클로드 시스템 프롬프트에 동적 결합하여 90% 이상의 품질 향상 달성.
